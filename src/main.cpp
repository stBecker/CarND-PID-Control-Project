        ...
        if (event == "telemetry") {
          ...
          */
          ...
          if (fabs(cte) >= safety_margin)
          {
            if (speed >= 10.0)
            {
              throttle = std::max(-(fabs(cte) - safety_margin) / (safety_margin*3.0), -1.0);
            }
            else
            {
              throttle = 0.3;
          ...
          // DEBUG
          std::cout << "CTE: " << cte << " Steering Value: " << steer_value << " Previous Angle: " << angle << std::endl;
